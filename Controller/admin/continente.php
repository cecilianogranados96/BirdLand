...
###################################################################
    if(isset($_GET['borrar'])) {
        $stid = oci_parse($conn, 'BEGIN pck_continente.delete_continente('.$_GET['borrar'].'); END;');
    ...
    if(isset($_GET['nuevo'])) {
        $stid = oci_parse($conn, "BEGIN pck_continente.insert_continente('".$_POST['nombre']."'); END;");
        oci_execute($stid);
    ...
    if(isset($_GET['editar'])) {
        $stid = oci_parse($conn, "BEGIN pck_continente.update_continente('".$_GET['id']."','".$_POST['nombre']."'); END;");
        oci_execute($stid);
        $mensaje = ' <div class="alert alert-success"> 
    ...
    if(isset($_GET['edit'])) {     
        error_reporting(0);
        $stid = oci_parse($conn, "begin :r := pck_continente.continente_nombre(".$_GET['id']."); end;");
    ...
    {  
       ...
       $filas .=  '<tr>
                ...
                <!--<td><center>CLASE</td> -->
                <td><center>
                      <a href="?pag=admin/continente&edit=1&id='.$row['ID_CONTINENTE'][$i].'" class="btn  btn-success">Editar</button>
                </td>
              </tr>';
    }
